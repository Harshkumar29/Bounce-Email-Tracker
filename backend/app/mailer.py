...
def _send_one(campaign: models.Campaign, recipient: models.Recipient, tracking_urls: dict) -> None:
    ...
    try:
        ...
        recipient.bounce_reason = str(exc)[:2000]
        recipient.bounced_at = now_utc()
        return

    except smtplib.SMTPResponseException as exc:
        recipient.bounced = True
        recipient.bounce_type = "soft" if 400 <= exc.smtp_code < 500 else "hard"
        recipient.bounce_reason = str(exc.smtp_error)[:2000]
        recipient.bounced_at = now_utc()
        return

    except (smtplib.SMTPException, OSError) as exc:
        recipient.bounced = True
        recipient.bounce_type = "soft"
        recipient.bounce_reason = str(exc)[:2000]
        recipient.bounced_at = now_utc()
        return
